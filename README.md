...
1，createTimeSegments 有问题，对比逻辑更改后，时间分段时，每5分钟一段就好。 基准表每5分钟的数据，对比另外一张表的5分钟+前后各2s的数据。且基准数据取到第一条数据后，待对比数据取其时间前后2s的所有数据，看是否能够完全匹配，能匹配则从待对比数据列表中消除时间最接近的那一条，同时，基准数据中也删除该条数据。 全部对比完成后，基准表中未被消除的数据，打印出来，待对比数据中，如果在时间开始结束区间前后2s内的数据可忽略，除次之外，还有未删除的数据，也需要打印出来。
2，每次查询5分钟的数据，不需要分页
3， 打印方式也调整下，每5分钟，打印一次。
